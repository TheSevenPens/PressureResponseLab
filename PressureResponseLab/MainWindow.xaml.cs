...
public partial class MainWindow : Window
{
    ...
    private const int PenPollIntervalMs = 8;
    ...
    private DispatcherTimer? penPollTimer;
    ...
    public MainWindow()
    {
        ...
        var penSession = PenSessionFactory.Create(InputApi.WintabSystem);
    ...
    private void ProcessPenPoint(PenPoint pt)
    {
        if (pt.ButtonAction != PenButtonAction.None)
        {
            bool pressed = pt.ButtonAction == PenButtonAction.Pressed;
            if (pt.ButtonNumber == PenButtonNumber.Tip)
            {
                checkBox_tipdown.IsChecked = pressed;
            }
            else if (pt.ButtonNumber == PenButtonNumber.Barrel1)
            {
                checkBox_lowerbuttondown.IsChecked = pressed;
            }
            else if (pt.ButtonNumber == PenButtonNumber.Barrel2)
            {
                checkBox_upperbuttondown.IsChecked = pressed;
            }
        ...
        uint raw_pressure = pt.Pressure;
        int max_pressure = appstate!.PenSession!.MaxPressure;
        double normalized_raw_pressure = max_pressure > 0
            ? raw_pressure / (1.0 * max_pressure)
            : 0.0;

        appstate.LogicalPressure = normalized_raw_pressure;

        label_pressure_raw.Content = raw_pressure.ToString();
        label_normalized_pressure.Content = $"{normalized_raw_pressure * 100.0:00.000}%";

        label_or_altitude.Content = $"{pt.Altitude:F0}°";
        label_or_azimuth.Content = $"{pt.Azimuth:F0}°";
        label_tiltx.Content = $"{pt.TiltX:00.000}°";
        label_tilty.Content = $"{pt.TiltY:00.000}°";
    ...
    private void PenPollTimer_Tick(object? sender, EventArgs e)
    {
        var session = appstate?.PenSession;
        if (session is null || !session.IsRunning || !session.HasNewData)
        {
            return;
        }
    ...
    private void StartPenSession()
    {
        try
        {
            IntPtr hwnd = new WindowInteropHelper(this).Handle;
            string status = appstate!.PenSession!.Start(hwnd);
            if (!string.IsNullOrEmpty(status))
            {
                // Status text returned by the session (often diagnostic info)
                System.Diagnostics.Debug.WriteLine($"PenSession.Start: {status}");
            }

            penPollTimer = new DispatcherTimer(DispatcherPriority.Input)
            {
                Interval = TimeSpan.FromMilliseconds(PenPollIntervalMs)
            };
            penPollTimer.Tick += PenPollTimer_Tick;
            penPollTimer.Start();
        }
        catch (Exception ex)
        {
            MessageBox.Show($"Failed to initialize pen session: {ex.Message}", "Initialization Error");
        }
    }

    private void StopPenSession()
    {
        if (penPollTimer is not null)
        {
            penPollTimer.Stop();
            penPollTimer.Tick -= PenPollTimer_Tick;
            penPollTimer = null;
        }
        appstate?.PenSession?.Stop();
    }
